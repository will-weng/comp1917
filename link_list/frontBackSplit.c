...
void frontBackSplit (list sourceList, list frontList, list backList) {
   ...
   if(sourceList->head != NULL) {
      link currNode = sourceList->head;
      frontList->head = sourceList->head;
      link frontListEnd = NULL;
      int counter = numItems(sourceList);

      if(counter % 2 == 0) {
         counter = (numItems(sourceList)/2);
      } else {
         counter = (numItems(sourceList)/2)+1;
      }
      while(counter != 1) {
      ...
      frontListEnd = currNode;
      currNode = currNode->next;
      frontListEnd->next = NULL;
      backList->head = currNode;
